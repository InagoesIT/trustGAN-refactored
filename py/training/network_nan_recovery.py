...
class NetworkNaNRecovery:
    ...
    def recover_from_nan_target_model(self):
        ...
        if torch.any(torch.isnan(outputs)):
            ...
            networks = glob.glob("{}/networks/target_model-step-*.pth".format(self.path_to_save))
            networks = sorted(sorted(networks), key=len)

            idx = -1

            while torch.any(torch.isnan(outputs)):

                self.networks_data.target_model.load_state_dict(torch.load(networks[idx]))

                outputs = self.networks_data.target_model(inputs)

                idx -= 1

            print(f"WARNING: Recover a proper state there: {networks[idx + 1]}")
    ...
    def recover_from_nan_gan(self):
        ...
        if torch.any(torch.isnan(gan_outputs)):
            ...
            networks = glob.glob(
                ...
                "{}/networks/gan-not-is_best-step-*.pth".format(self.path_to_save)
            )
            ...
            while torch.any(torch.isnan(gan_outputs)):
                ...
                self.networks_data.gan.load_state_dict(torch.load(networks[idx]))

                gan_outputs = self.networks_data.gan(rand_inputs)

                idx -= 1

            print(f"WARNING: Recover a proper state there: {networks[idx]}")
